    ...
    #pragma region HTTP
        ...
        int handle_http_request(HTTPServer* server, ClientSocket client_socket) {
            ...
            printf("%ld", server->routes_length);
            for (size_t r = 0; r < server->routes_length; r++) {

                printf("%s : %s", uri, server->routes[r].address);
                if (strcmp(uri, server->routes[r].address) == 0) {
                    ...
                    write(client_socket, result.content, result.length);
                    close(client_socket);
                    return 0;
            ...
            write(client_socket, CFRAME_404_ERROR, strlen(CFRAME_404_ERROR));
            close(client_socket);
            return 1;
